...
def service_week_screen_view(request):
    ...
    if request.user.is_authenticated:
        ...
        current_week_dates_list = list(_get_current_week_dates_list())
        ...
        current_week_service_occurrences_days = [
            ...
            for current_week_service_occurrence in current_week_service_occurrences
        ]
        ...
        for current_week_date in current_week_dates_list:
            if current_week_date in current_week_service_occurrences_days:
                for (
                    current_week_service_occurrence
...
def list_territories_view(request):
    ...
    if request.user.is_authenticated:
        ...
        for territory in all_territories:
            ...
            if t_last_service_occurrence:
                ...
                t_is_forgotten = last_service_occurrence_date.strftime(
                    "%Y-%m-%d"
                ) <= date_30_days_ago.strftime("%Y-%m-%d")
        ...
        return render(
            request, "outerpages/list_territories.html", {"territories": territories}
        )
...
def edit_territory_view(request, id):
    ...
    if request.user.is_authenticated and request.user.is_staff:
        territory = Territory.objects.get(id=id)
        return render(
            request, "outerpages/edit_territory.html", {"territory": territory}
        )
...
def new_service_occurrence_view(request):
    ...
    if request.user.is_authenticated and request.user.is_staff:
        ...
        leaders = JWAdminUser.objects.filter(congregation=request.user.congregation.id)

        return render(
            request,
            "outerpages/new_service_occurrence.html",
            {"territories": territories, "leaders": leaders},
        )
