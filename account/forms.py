...
class RegistrationForm(UserCreationForm):
    ...
    congregation = forms.ChoiceField(
        label="Congregação",
        choices=[(0, "Selecione sua congregação")] + congregation_choises,
    )
    congregation.widget.attrs.update({"class": "form-control"})
    ...
    password2 = forms.CharField(
        label="Digite novamente sua senha", widget=forms.PasswordInput
    )
    ...
    def clean(self):
        ...
        if not authenticate(email=email, password=password):
            raise forms.ValidationError(
                "Login inválido, confira o email ou o password."
            )
